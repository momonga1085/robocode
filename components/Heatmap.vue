<template id="line">
  ...
  >
    ...
    >
      ...
      <g ref="brushes" :transform="transform(margin.left, margin.top )">
...
export default {
  ...
  methods: {
    ...
    getScaleX() {
      let from = "2007-10-15 8:00:00"
      let to = "2007-10-15 10:00:00"
      var xScale = d3
        .scaleTime()
        .domain([
          new Date(from),
        ...
        .range([0, this.width]);
        let divNumber = (new Date(to) - new Date(from))/(60*1000)/1
        this.cellWidth = this.width/divNumber
    ...
    drawChart() {
      ...
      const brushX = d3
        ...
        .on("end", this.brushed);
      var brushElm = d3.select(this.$refs.brushes);
      brushElm.call(brushX);
      // this.drawLine();
      // this.drawPoint();
    },
    brushed() {
      let xScale = this.getScaleX();
      console.log(xScale.invert(d3.event.selection[0]));
    },
